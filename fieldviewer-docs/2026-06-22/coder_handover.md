> Public documentation snapshot: 2026-06-22. Sanitized from the private FieldViewer repository docs for the public Y1 demo. Private engineering instance names, local paths, internal server addresses, and server-only environment paths were redacted.
...
The current and old machines can both run Codex against FieldViewer through the
same GitHub account. Use commits and pushes as the control handoff; do not rely
on generated files or local data folders syncing automatically.
...
If the branch is both ahead and behind, or if unrelated local files are dirty,
pause and inspect the exact diff before continuing. Avoid simultaneous edits to
the same source files from both machines. After a task, run the relevant
validation/build commands, commit only intentional source/docs/manifest changes,
push, then have the other machine pull before resuming.

Do not commit credentials or machine-local files. `key/`, `.env`, SSH keys,
API keys, local GitHub credentials, and deployment secrets must stay outside
Git. Shared Git credentials should live in Git Credential Manager, GitHub CLI,
or the Windows user profile.
...
- `tools/convert_dashboard_input_to_fieldviewer.py`
- `tools/init_db.py`
...
Convert the newer private engineering instance dashboard workbook to the existing FieldViewer production
and well-test schemas:

```powershell
python tools\convert_dashboard_input_to_fieldviewer.py --dry-run
python tools\convert_dashboard_input_to_fieldviewer.py
python tools\validate_instance.py instances\private engineering instance\fieldviewer.instance.json
python tools\rebuild_site.py --instance-config instances\private engineering instance\fieldviewer.instance.json
```

Expected current private engineering instance conversion/build behavior:

- production input converts to 284,222 rows
- well-test input converts to 17,629 rows before app filtering
- generated Production page builds with 364 wells and 767 months
- generated Well Testing page builds with 289 wells and 17,620 usable tests
- backups are written under `instances/private engineering instance/Data/_backups/dashboard_input_conversion/`

The converter uses `DH_Well_ID.csv` plus an explicit zero-padding alias
dictionary. It preserves unresolved well names in the converted files and JSON
...
private engineering instance dashboard input conversion:

- source workbook: `instances/private engineering instance/Data/Dashboard_data_input_20260428.xlsx`
- useful source sheets: `Values`, `Well_tests`
- production target: `instances/private engineering instance/Data/private engineering instance - All_Prod_Data and New Columns private engineering instance 23Nov2023.xlsx`
- well-test target: `instances/private engineering instance/Data/well_test_summary_normalized.csv`
- catalog/enrichment source: `instances/private engineering instance/Data/DH_Well_ID.csv`
- generated HTML remains output and should be regenerated, not edited by hand

Instance `Data/` and generated `App/` paths are ignored by Git. If the converter
or generator changes are pushed, the other machine must rerun conversion and
rebuild locally with its local instance data.
