...
## 2026-06-02 current update

Two Codex instances may now share FieldViewer control through GitHub from the
current and old machines. The current-machine checkout is:

```text
[local FieldViewer workspace]
```

The old-machine checkout may still be:
...
If the branch is ahead and behind or the tree is dirty, inspect before pulling
or pushing. Do not reset or revert another agent's work. GitHub carries source,
manifests, and docs; instance `Data/` and generated `App/` outputs are ignored
and must be regenerated locally.

private engineering instance dashboard input conversion is now source-driven through:
...
The converter reads `instances/private engineering instance/Data/Dashboard_data_input_20260428.xlsx`
(`Values` and `Well_tests`), uses `DH_Well_ID.csv` for conservative well-name
matching and enrichment, applies only explicit zero-padding aliases, preserves
unresolved names, and writes backups under
`instances/private engineering instance/Data/_backups/dashboard_input_conversion/`.
