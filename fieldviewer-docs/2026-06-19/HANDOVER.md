...
- Current-machine repo: `[local FieldViewer workspace]`
- Old-machine repo may still be: `[local FieldViewer workspace]`
...
The current and old machines can share FieldViewer control through GitHub with
the same Git credentials, but Git is the only reliable handoff channel. Before
either Codex instance starts work:
...
If `git status --short --branch` shows both `ahead` and `behind`, reconcile the
branch deliberately before new work. Do not run destructive cleanup commands on
a dirty tree. Commit only the files intentionally changed for the task, and
push before the other machine resumes.

The repo intentionally ignores instance `Data/` and generated `App/` outputs.
When one machine changes a source tool or manifest, the other machine must pull
the source change and rerun the documented conversion/rebuild locally.
...
Convert the current private engineering instance dashboard workbook into the legacy-compatible
production and well-test inputs:
...
- `instances/private engineering instance/Data/Dashboard_data_input_20260428.xlsx`
- `Values` for monthly production
- `Well_tests` for well-test rows
- `instances/private engineering instance/Data/DH_Well_ID.csv` for conservative well-name matching

It writes the existing FieldViewer-compatible inputs:

- `instances/private engineering instance/Data/private engineering instance - All_Prod_Data and New Columns private engineering instance 23Nov2023.xlsx`
- `instances/private engineering instance/Data/well_test_summary_normalized.csv`

It creates timestamped backups under
`instances/private engineering instance/Data/_backups/dashboard_input_conversion/`. Alias handling is
explicit and zero-padding-only; unresolved well names are preserved and reported
rather than guessed.
